...
index = 0
for m in model.modules():   #遍历模型
    if isinstance(m, nn.BatchNorm2d):#找到BN层
        size = m.weight.data.shape[0]   #判定m（batchsize）的权重中的γ个数
        bn[index:(index+size)] = m.weight.data.abs().clone()    #取出γ值放入bn中
        index += size

y, i = torch.sort(bn)
thre_index = int(total * args.percent)  #截断值（按照百分比来乘以total） 进行剪枝
thre = y[thre_index]    #实际的阈值

#阈值的比较
pruned = 0
cfg = []
cfg_mask = []
for k, m in enumerate(model.modules()): #遍历网络 构建剪枝网络结构
    if isinstance(m, nn.BatchNorm2d):   #如果是BN层
        weight_copy = m.weight.data.clone() #拿到权重
        mask = weight_copy.abs().gt(thre).float().cuda() #.gt 比较前者是否大于后者（前者是否大于阈值）
        pruned = pruned + mask.shape[0] - torch.sum(mask)   #每层剪枝的个数
...
def test():
    ...
    test_loader = torch.utils.data.DataLoader(test_dataset, batch_size=16, shuffle=True, drop_last=False)
    # test_loader = torch.utils.data.DataLoader(
    #     datasets.Test('./data', train=False, transform=transforms.Compose([
    #         transforms.ToTensor(),
...
for [m0, m1] in zip(model.modules(), newmodel.modules()):   #取出每一层新旧权重 进行赋值
    #判断是哪些层 进行对应复制操作
